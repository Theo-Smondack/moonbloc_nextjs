...
import { Cryptos } from '../../../helpers/allCryptos'
  ...
  const searchCrypto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 0) {
      const filteredData = Cryptos.filter((crypto) =>
        crypto.name.toLowerCase().startsWith(value.toLowerCase())
      ).slice(0, 5)
      await fetch(
        `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${filteredData
            ...
            {searchValue.length > 0
              ? searchValue.map((value, index) => (
                  ...
                  >
                    ...
                    <div className={`${styles.searchResultSymbol} txtGrey`}>
                      {value.symbol}
                    </div>
                  </div>
                ))
