...
const AssetsList = ({Assets}: AssetListProps) => {
    ...
    return (
        <div className={styles.container}>
            {showModal ?
                <TransactionModal showCallback={setShowModal} Asset={asset as Asset} setAsset={setAsset}/> : null}
                    ...
                    updatedAssets.map((asset: Asset, index: number) => {
                            return <tr key={index}>
                                <td>
                                    ...
                                    <span className={'txtGrey'}>{asset.symbol.toUpperCase()}</span>
                                </td>
                                <td>{`${asset.price.toFixed(2)} ${currencySymbol}`}</td>
                                <td>{asset.quantity}</td>
                                <td>{`${asset.total.toFixed(2)} ${currencySymbol}`}</td>
                                <td><FontAwesomeIcon icon={faCirclePlus} className={styles.icon}
                                                     onClick={() => handleShowModal(asset)}/></td>
                            </tr>
                        }
                    )
                }
