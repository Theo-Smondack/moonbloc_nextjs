    ...
    return (
      <div className={styles.container}>
        <div className={styles.timeFrameContainer}>
          <div
            className={timeframe === '1' ? styles.active : ''}
            onClick={() => setTimeframe('1')}
          >
            24h
          </div>
          <div
            className={timeframe === '7' ? styles.active : ''}
            onClick={() => setTimeframe('7')}
          >
            7d
          </div>
          <div
            className={timeframe === '30' ? styles.active : ''}
            onClick={() => setTimeframe('30')}
          >
            1m
          </div>
          <div
            className={timeframe === '90' ? styles.active : ''}
            onClick={() => setTimeframe('90')}
          >
            3m
          </div>
          <div
            className={timeframe === '180' ? styles.active : ''}
            onClick={() => setTimeframe('180')}
          >
            6m
          </div>
          <div
            className={timeframe === '365' ? styles.active : ''}
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: function (context: any) {
            return `${(
              Math.floor(context.dataset.data[context.dataIndex] * 10000) /
              10000
            ).toLocaleString()}${value.state.currency?.symbol}`
          },
        },
      },
      legend: {
