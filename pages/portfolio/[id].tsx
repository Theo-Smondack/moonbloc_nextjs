...
import AssetSelectorModal, {
  searchValue,
} from '../../components/portfolio/portfolioPage/assetSelectorModal'
...
const PortfolioPage: NextPageWithLayout = () => {
  ...
  useEffect(() => {
    if (id) {
      setUrl(
        `/api/user/wallet/find/${id}?currency=${
  ...
  const { walletTitle, assets } = portfolio
  return (
    <div className={styles.container}>
      {showModal ? (
        <AssetSelectorModal
          showCallback={openModal}
          title={`Add crypto to ${walletTitle}`}
          addAsset={addAsset}
        />
      ) : null}
      <div className={styles.headContainer}>
        <h1>{walletTitle}</h1>
        <button
          className={`bgBlueButton ${styles.addTransactionButton}`}
      ...
      {assets.length ? (
        ...
        <div className={styles.noAssetContainer}>
          There is no cryptocurrencies in this portfolio
        </div>
      )}
    </div>
  )
