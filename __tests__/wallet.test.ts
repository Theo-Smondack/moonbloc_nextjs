...
describe('Wallet model', () => {
  ...
  describe('Create wallet', () => {
    ...
    describe('given input is not valid', () => {
      it('should throw an Error: "Wallet already exist"', async () => {
        ...
        await expect(newWallet).rejects.toThrow(
          new Error('Wallet already exist')
        )
      })
      it('should throw an Error: "This user already has the maximum of wallets"', async () => {
        ...
        await expect(newWallet).rejects.toThrow(
          new Error('This user already has the maximum of wallets')
        )
      })
    })
  })
  ...
  async function addTransactions(wallet: WalletDocument) {
    await createTransaction(
      {
        date: new Date(2022, 2, 12, 1, 35, 5),
        fee: 0.02,
        from: 'usd',
        price: 120,
        quantity: 1,
    ...
    await createTransaction(
      {
        date: new Date(2023, 0, 25, 13, 14, 46),
        fee: 0.02,
        from: 'usd',
        price: 120,
        quantity: 1,
    ...
    await createTransaction(
      {
        date: new Date(2023, 1, 20, 10, 54, 18),
        fee: 0.02,
        from: 'ethereum',
        price: 250,
        quantity: 0.745,
  ...
  describe('Update wallet', () => {
    it('should update a wallet', async () => {
      ...
      expect(updatedWallet.walletTitle).toBe('New wallet title')
    })
  })
